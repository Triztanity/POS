...
class _PassengersScreenState extends State<PassengersScreen>
  ...
  Map<String, int> getPassengersByDestination() {
    ...
    final currentIdx =
        currentLocation == 'OVERVIEW' ? -1 : stops.indexOf(currentLocation!);
      ...
      body: SafeArea(
        child: Padding(
          padding: EdgeInsets.symmetric(
              horizontal: screenW * 0.03, vertical: screenH * 0.015),
          child: Column(
            ...
            children: [
              ...
              const Text('Scheduled Drop-offs',
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    const Text('Trip Revenue',
                        style: TextStyle(fontWeight: FontWeight.bold)),
                    Text('₱${_formatRevenue(_getTripRevenue())}',
                        style: const TextStyle(
                            fontWeight: FontWeight.bold,
                            fontSize: 16,
                            color: Colors.green)),
  ...
  Widget _buildDropoffsTableCompact() {
    ...
    final entries = countsByStop.entries
        .where(
            (e) => ((e.value['walkin'] ?? 0) + (e.value['booking'] ?? 0)) > 0)
        .toList();
  ...
  double _getTripRevenue() {
    ...
    double total = 0.0;
    for (final b in bookings) {
      // `amount` is already the total ticket amount for the booking/walk-in.
      // Do not multiply by passengers here to avoid double-counting.
      if (b.status == 'on-board') {
        total += b.amount;
      }
    }
